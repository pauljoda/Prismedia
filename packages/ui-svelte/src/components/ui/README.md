...
- Global search composes Command Input, List, Group, and Item with the shared shadcn Dialog. The shared thumbnail still owns artwork; the app owns request debouncing, stale-response invalidation, history, and navigation. Disable Command filtering for server-ranked results and use its public selection API after asynchronous rows mount. Recent and full-result actions belong in the same keyboard list, with removal buttons outside the option itself.
...
## Shared form and state owners

- ChoicePicker composes Popover and Command for SearchSelect, TagSelect, and EntityPicker. App wrappers retain catalog filtering, entity fetching, creation policy, and persisted selections. Async searches invalidate old requests as soon as the query changes and expose loading, error, and retry states.
- Checkbox reports a boolean through `onchange`, not a synthetic native event. Function binding keeps the parent authoritative, including mixed select-all states. ToggleButton is a pressed action; Toggle remains the Switch adapter.
- PasswordField uses InputGroup with a keyboard-accessible reveal button. DateField retains the platform date picker through TextInput. ListEditor, KeyValueEditor, CreditsEditor, and MarkdownEditor build their control surfaces on shared inputs, buttons, badges, and toggles; their domain/editing logic stays in the app.
- Progress exposes accessible progress values. Meter adds display text and clamping. Artwork-aware callers can set `--progress-fill` without copying progress markup. Empty, Alert, Skeleton, and Table own ordinary empty/error/loading/tabular presentation; branded loaders and media-specific visualizations remain product components.
- Sheet owns mobile navigation's modal behavior. The app retains navigation customization and the existing drag-to-dismiss gesture; `overlayProps` synchronizes the backdrop with that gesture. Reader and player engines are not UI-library replacements.
- ReviewSection uses Collapsible; collection choices use RadioGroup and ToggleGroup. Keep field choices and selection ordering in their existing domain owners.
- ReaderShell and UniversalLightbox compose the fullscreen Dialog adapter. Reader contents use Sheet; player settings use Popover anchored at the actual settings control. Keyboard navigation is scoped to its reader and yields to composite controls and higher overlays. Native fullscreen popovers use the fullscreen container as their portal host.
- Ratings, filter chips, and chart-legend selections use ToggleButton, retaining domain labels, colors, and selection rules. Function bindings keep persisted or asynchronous values authoritative until the caller accepts a change. File, job, Identify, transcript, and navigation actions use Button; artwork tiles may retain their domain-specific layout on that base.
- Production Svelte templates do not define raw buttons, selects, textareas, dialogs, or ordinary inputs. The source ownership test enforces this boundary. Native file-upload inputs remain inside their domain upload controls; browser file access is not replaced by a component library. Vidstack, foliate, PDF.js, the file-tree renderer, and drag/reorder engines retain their specialized rendering and interaction responsibilities.
...
- Portal placement uses the nearest native dialog when present. Body portals cannot escape a native modal's inert boundary. The shared Dialog now uses Bits UI for focus, Escape, outside dismissal, and scroll locking; it no longer adds native modal or window-event machinery.
- Dialog content portals mount in opening order so independent dialogs, including global search over an editor, paint in the same order as the focus/Escape stack. Busy callers reject dismissal. The optional `initialFocus` callback selects a task field once per opening. A top-aligned dialog must also clear the base vertical centering transform.
...
Global search tests cover recent-search keyboard activation, preserved server ranking, per-kind limits, full-result actions, navigation/history, stale responses during debounce, empty/error/retry states, and action-key isolation. Validate Escape/focus return, pointer interaction after dismissal, and nested-dialog layering in the running app; JSDOM cannot prove top-layer rendering.

Also inspect `/design-language#component-bases` and a real Settings section through the .NET app. Test pointer selection, keyboard selection, scrolling, focus return, layered Escape, and a narrow viewport. DOM tests do not establish visual stacking, touch behavior, or visual quality.
