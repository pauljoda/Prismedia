...
description: >
  Prismedia's video-first remix of Broadside. The unit is the frame (1920×1080). Atoms are
  true-black ground, neutral-silver typography, Geist Sans display, JetBrains Mono chrome,
  sharp editorial geometry, sparse material-spectrum identity, and 1px hairline dividers.
  Composition + frame scale are rewritten for the frame. Motion is out of scope.
...
colors:
  ...
  text: "#F5F5F6"
  ink-black: "#000000"
  ink-black-alt: "#09090A"
  fire-orange: "#B3484D"
  cream: "#F5F5F6"
  cream-muted: "#C8C9CC"
  cream-hint: "#92959D"
  border-dark: "rgba(164,172,185,0.16)"
  surface-1: "#111214"
  surface-2: "#191A1E"
  prism-red: "#B3484D"
  prism-orange: "#B76337"
...
typography:
  # — reading ramp —
  body:    { fontFamily: "Geist Sans", cqw: 1.2, weight: 400, lineHeight: 1.6 }
  lead:    { fontFamily: "Geist Sans", cqw: 1.6, weight: 400, lineHeight: 1.5 }
  caption: { fontFamily: "Geist Sans", cqw: 0.9, weight: 400, lineHeight: 1.5 }
  label:   { fontFamily: "JetBrains Mono", cqw: 0.72, weight: 500, tracking: "0.14em", upper: true }
  # — display / hero ramp (Geist Sans, lowercase, negative tracking) —
  h3:      { fontFamily: "Geist Sans", cqw: 2.8, weight: 600, lineHeight: 1.2, lower: true }
  quote-text:{ fontFamily: "Geist Sans", cqw: 3.8, weight: 700, lineHeight: 1.15, tracking: "-0.02em", lower: true }
  h2:      { fontFamily: "Geist Sans", cqw: 4.5, weight: 700, lineHeight: 1.1, tracking: "-0.02em", lower: true }
  stat-value:{ fontFamily: "Geist Sans", cqw: 5.5, weight: 800, lineHeight: 1.0, tracking: "-0.04em" }
  h1:      { fontFamily: "Geist Sans", cqw: 7.5, weight: 800, lineHeight: 0.9, tracking: "-0.03em", lower: true }
  fadelist-item:{ fontFamily: "Geist Sans", cqw: 7.5, weight: 800, lineHeight: 1.0, tracking: "-0.03em", lower: true }
  quote-mark:{ fontFamily: "Geist Sans", cqw: 10.0, weight: 800, lineHeight: 0.6 }
  fadelist-title:{ fontFamily: "Geist Sans", cqw: 10.5, weight: 800, lineHeight: 0.9, tracking: "-0.04em", lower: true }
  display: { fontFamily: "Geist Sans", cqw: 13.0, weight: 800, lineHeight: 0.88, tracking: "-0.04em", lower: true }
...
components:
  registers:
    ...
    silver: "ground {colors.cream-muted}, text {colors.ink-black}; rare declarative reversal only"
    spectrum: "identity rays only: red → orange → yellow → green → cyan → blue → violet; never a full-frame gradient wash"
    description: "True black is the default ground. Silver reversal is rare. Spectrum color is sparse identity, never generic ambience."
  slide-chrome:
...
- **Fonts** — already set to **Geist Sans** (display) / **Inter Variable** (body); ignore any preset font name lingering in prose.
- **Chrome** — already set to **JetBrains Mono**; ignore IBM Plex Mono references in the inherited prose.
...
- **Colors** — use the frontmatter hex. The default ground is true black and type is neutral silver/white. `fire-orange` is the inherited preset's single-accent slot and maps to Prismedia material red.
- **Spectrum** — the eight `prism-*` tokens are reserved for the literal refraction motif, selected-state rails, and one-beat handoffs. Never turn them into an ambient rainbow gradient or repeat every hue around every surface.
- **Surface** — real product captures keep Prismedia's controlled radii and glass chrome. Broadside's zero-radius, flat-plane rule applies to authored editorial framing around those captures, not to repainting the product UI.


## Overview

Broadside at frame scale is a **protest-poster system where type is so large it stops reading as
text and becomes graphic primitive.** Geist Sans `display` at 13cqw puts a single lowercase word
...
**Geist Sans** carries every text role from display to body — expressive range from weight (400–900)
...
- **Massive lowercase Geist Sans 900**, negative-tracked, as graphic primitive (display 13cqw).
...
Two ramps. The **reading ramp** (Geist Sans body 1.2cqw, mono label 0.72cqw) carries copy + chrome; the
**display ramp** (Geist Sans `h2` 4.5cqw → `display` 13cqw, weight 700–900) carries every statement.
...
- **Geist Sans display is lowercase, weight 700–900, negative-tracked** (−0.04em largest, −0.02em h2). **Mono chrome is uppercase, 0.1em+.** No italic, no underline, no uppercase display.
...
Geist Sans `display` (13cqw) lowercase in ink, left-anchored, over a small ink rule stub + mono kicker; a
Geist Sans lead line beneath in 75% ink. **Chrome** mono catalogue number top-left, mono meta top-right
...
**Ground** ink-black. **Composes** kicker, display. **Focal** a 2–4 word Geist Sans `display`/`h1`
...
row of three top-border-only stat-cards — big Geist Sans-900 numeral in `{colors.fire-orange}`, Geist Sans
...
**Ground** ink-black. **Composes** fadelist (3 stacked Geist Sans-900 words at 1.0/0.5/0.22), fadelist-title.
...
Geist Sans `quote-text` (700, lowercase) at ≤78cqw under an oversized fire-orange `quote-mark` (10cqw,
...
- Set every Geist Sans display in **lowercase weight 900**, negative-tracked — the system's signature.
...
- Never uppercase Geist Sans display; never add a second accent color.
...
- No serif companion; chrome is never Geist Sans.
...
- **Type** — Geist Sans lowercase 900 negative-tracked, fit-to-measure; mono chrome uppercase 0.14em; ≥1.4cqw floor.
...
- **Geist Sans + IBM Plex Mono via Google Fonts**; Noto Sans SC is the CJK fallback (the lowercase-display signal has no CJK equivalent — the two-register color system carries the identity, per the source).
