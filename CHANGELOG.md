...
- Added author requests: search an author in Discover, open their detail page, and toggle exactly which of their books to get before requesting — each selected book becomes its own acquisition (the author itself is never downloaded). This mirrors how a series lets you pick volumes, so requesting a whole author's catalog, or just a few titles, is one flow. Author results are limited to providers that can also supply the books.
