...
### Fixed
- Transcoded video no longer stalls after a few seconds of playback. The player was buffering minutes ahead, which made the server start a separate transcode for each far-ahead chunk; those parallel transcodes fought over the hardware encoder and all fell below real time. The player now keeps a bounded look-ahead and the server funnels each stream onto a single forward transcode (reused as the player buffers, restarted on a seek), so a 4K Dolby Vision title transcodes comfortably faster than real time and plays smoothly.
