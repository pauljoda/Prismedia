...
## [Unreleased]
### What's New
- Books are now more than comics. EPUB books are scanned as first-class library items — their cover, title, and author are read straight from the file — and open in a built-in reflowable reader. The reader remembers exactly where you left off (down to the spot on the page), lets you switch between paged and scrolling layouts and adjust the text size, and shows the current chapter in the top bar. Comics continue to read exactly as before.
...
### Added
- Added EPUB books as a single-file book format: `.epub` files scan into one book entity each (no chapter/page entities), with title/author/description and the embedded cover image read from the file, and a reflowable in-app reader (paged/scrolled layouts, adjustable text size, and chapter readout) that streams the source file and resumes from a saved per-book reading location.
