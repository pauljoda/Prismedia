...
- Audiobook searches prefer releases with exact chapter structure: a single M4B ranks first, then one file per chapter, then releases whose layout is unknown, then part splits, and finally one long MP3. Releases that offer only audio Prismedia cannot import, such as FLAC, Opus, or AAX, are skipped instead of failing after download. A download that mixes formats imports one coherent set (M4B before MP3), and multi-disc downloads keep disc then track order instead of interleaving same-named tracks.
