...
## [Unreleased]
### What's New
- The audio player is now a single persistent bar that keeps playing as you browse — it no longer stops when you leave the album page. Press Play or Shuffle on an album (or play any track) and it picks up a real play queue: a new queue button opens an "Up Next" list you can see and click to jump ahead, and Shuffle now builds a fixed shuffled order you can look through rather than jumping to a random track each time. The now-playing artist is shown and links to the artist page (clicking it keeps the music going), and track tiles show their track number. Artist pages now have their own Play All and Shuffle that queue every track across all of the artist's albums.
