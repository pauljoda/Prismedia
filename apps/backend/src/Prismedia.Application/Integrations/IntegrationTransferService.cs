...
public sealed class IntegrationTransferService(IIntegrationTransferStore store, IJobQueueService queue) {
    ...
    public async Task<IntegrationTransferResponse> CancelAsync(Guid id, CancellationToken cancellationToken) {
        ...
        if (!transfer.Mode.IsSource) {
            if (!transfer.CanCancelRemote && !transfer.State.CancellationRequested
                && transfer.State.Phase != IntegrationTransferPhase.Cancelled) {
            ...
            if (transfer.State.Revision != expected) {
                await store.SaveAndEnqueueAsync(transfer, expected, cancellationToken);
            } else if (transfer.State.Phase != IntegrationTransferPhase.Cancelled) {
                await store.EnqueueRetryAsync(id, cancellationToken);
            }
        ...
        if (!transfer.CanCancelSource && transfer.State.Phase != IntegrationTransferPhase.Cancelled) {
            throw new ArgumentException(
                "This transfer has already begun library import and cannot be cancelled. Retry it to reconcile its files.");
        }
    ...
    internal static IntegrationTransferResponse ToResponse(StoredIntegrationTransfer work) {
        ...
        return new(state.OperationId, state.ConnectionId, work.Plan.Title, work.Plan.EntityKind, work.Plan.LibraryRootId,
            ...
            state.Imports?.SelectMany(imported => imported.ContainerEntityId is { } container ? new[] { container } : imported.EntityIds)
                .Distinct().ToArray() ?? [], work.LastError,
