...
public sealed class ManagedLibraryService(IntegrationConnectionAccess access, IIntegrationManagerGateway gateway) {
    ...
    public async Task<ManagedLibraryPage> SearchAsync(Guid connectionId, ManagedLibraryQuery input, CancellationToken cancellationToken) {
        ...
        var authorized = await access.RequireAsync(connectionId, PluginCapability.ConnectedLibrary, IntegrationOperation.SearchLibrary,
            input.EntityKind, cancellationToken);
        ...
        if (page.Items is null || page.Items.Count > input.Limit || page.NextCursor?.Length > 8192
            || page.Items.Select(item => item?.RemoteId).Distinct().Count() != page.Items.Count) {
            throw Invalid();
        }

        foreach (var item in page.Items) {
            ValidateItem(item, input.EntityKind);
        }
    ...
    public async Task<ManagedItemSnapshot> GetAsync(Guid connectionId, ManagedItemInput input, CancellationToken cancellationToken) {
        ...
        var authorized = await access.RequireAsync(connectionId, PluginCapability.ConnectedLibrary, IntegrationOperation.GetLibraryItem,
            input.EntityKind, cancellationToken);
    ...
    public async Task<ManagerOptions> OptionsAsync(Guid connectionId, ManagerOptionsInput input, CancellationToken cancellationToken) {
        ...
        var authorized = await access.RequireAsync(connectionId, PluginCapability.ExternalManager, IntegrationOperation.ManagerOptions,
            input.EntityKind, cancellationToken);
        ...
        if (options.Profiles is null || options.Roots is null || options.Profiles.Count > 1000 || options.Roots.Count > 1000
            ...
            || options.Roots.Select(root => root.Id).Distinct().Count() != options.Roots.Count) {
            throw Invalid();
        }
    ...
    public static void ValidateSnapshot(ManagedItemInput input, ManagedItemSnapshot snapshot) {
        if (!IsValidInput(input) || snapshot is null) {
            throw Invalid();
        }

        ValidateItem(snapshot.Item, input.EntityKind);
        if (snapshot.Item.RemoteId != input.RemoteId
            || input.ExpectedExternalIds.Any(pair => snapshot.Item.ExternalIds.GetValueOrDefault(pair.Key) != pair.Value)
            || !Text(snapshot.Path, 8192) || snapshot.ObservedAt == default || snapshot.ObservedAt > DateTimeOffset.UtcNow.AddMinutes(5)
            || snapshot.Files is null || snapshot.Files.Count > 10000
            || snapshot.Files.Select(file => file?.RemoteId).Distinct().Count() != snapshot.Files.Count) {
            throw Invalid();
        }
        ...
        foreach (var file in snapshot.Files) {
            if (file is null || !Text(file.RemoteId, 512) || !Text(file.Path, 8192) || file.SizeBytes <= 0
                || file.Targets is not { Count: > 0 and <= 1000 }
                || file.Targets.Select(target => target?.RemoteId).Distinct().Count() != file.Targets.Count) {
                throw Invalid();
            }
            ...
            foreach (var target in file.Targets) {
                if (target is null || !Text(target.RemoteId, 512) || !Text(target.Title, 512) || !Enum.IsDefined(target.EntityKind)
                    ...
                    || target.IssueLabel is not null && (!Text(target.IssueLabel, 128) || expected?.Shape.RequiresIssueLabel != true)) {
                    throw Invalid();
                }
            }
        }
        ...
        if (targets.Select(target => target.RemoteId).Distinct(StringComparer.Ordinal).Count() != targets.Length
            || expected is not null && targets.Any(target => target.EntityKind != expected.Kind
                || !expected.Shape.Accepts(input.RemoteId, target.RemoteId, target.SeasonNumber, target.EpisodeNumber,
                    target.AbsoluteNumber, target.IssueLabel))) {
            throw Invalid();
        }

        if (snapshot.ComicIssues is { } issues) {
            if (expected?.Shape != ManagedTargetShape.Issue || issues.Count > 10000
                || issues.Any(issue => issue is null || !Text(issue.RemoteId, 512) || !Text(issue.IssueLabel, 128)
                    || !Text(issue.Title, 512)
            ...
            if (comicVineIds.Distinct(StringComparer.Ordinal).Count() != comicVineIds.Length) {
                throw Invalid();
            }
            ...
            if (targets.Any(target => !issueById.TryGetValue(target.RemoteId, out var issue)
                || target.IssueLabel != issue.IssueLabel || target.Title != issue.Title)) {
                throw Invalid();
            }
        }
    }
    ...
    private static void ValidateItem(ManagedLibraryItem item, EntityKind kind) {
        if (item is null || item.EntityKind != kind || !Text(item.RemoteId, 512) || !Text(item.Title, 512)
            ...
            || !ValidPresentation(item.Presentation)) {
            throw Invalid();
        }
    }
    ...
    private static IntegrationInvocationException Invalid() =>
        new("The connected library returned invalid, oversized, or mismatched holding evidence.");
