...
public sealed class SourceTransferProcessor(IIntegrationTransferStore store, CatalogDiscoveryService discovery,
    ...
    /// <summary>Verifies, places, and materializes exact publication bytes, persisting evidence between each recoverable
    /// boundary.</summary>
    public async Task ProcessAsync(Guid operationId, JobContext context, CancellationToken cancellationToken) {
        ...
        try {
            ...
            if (transfer.State.Phase == IntegrationTransferPhase.Transferring) {
                ...
                var resolved = await discovery.ResolveSelectionAsync(transfer.State.ConnectionId, source.Selection, source.OfferId,
                    cancellationToken);
                ...
                var origin = IntegrationDeliveryOriginPolicy.RequireAllowedOrigin(connection.Manifest.Integration,
                    connection.Context.BaseUrl, resolved.Delivery);
                ...
                transfer.AcceptSourceArtifact(new(artifact.ArtifactId, source.Selection.ItemId, artifact.FileName,
                    resolved.Offer.MediaType ?? "application/octet-stream", artifact.SizeBytes, artifact.Sha256,
                    IntegrationArtifactRole.Content));
                ...
                throw new InvalidOperationException("This publication cannot advance from its current phase.");
            }
            ...
            transfer.RecordImported(new(artifact.ArtifactId, artifact.Sha256,
    ...
    private async Task VerifyPublicationAsync(VerifiedIntegrationArtifact artifact, EntityKind kind, CancellationToken cancellationToken) {
        try {
            await verifier.VerifyAsync(artifact, kind, cancellationToken);
