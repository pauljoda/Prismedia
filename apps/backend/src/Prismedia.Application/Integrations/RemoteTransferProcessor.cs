...
/// <summary>Resumes durable executor fulfillment across submission ambiguity, remote waits, byte transfer, local import, and
...
public sealed class RemoteTransferProcessor(IIntegrationTransferStore store, IntegrationConnectionAccess access,
    ...
    /// <summary>Runs until a durable remote wait or local completion boundary. A wait releases the queue worker without consuming
    /// a failure attempt.</summary>
    public async Task ProcessAsync(Guid operationId, JobContext context, CancellationToken cancellationToken) {
        ...
        async Task<AuthorizedIntegrationConnection> AuthorizeAsync(IntegrationOperation operation) {
            ...
            if (!connection.Connection.State.HasPersistentRemoteIdentity
                || connection.Context.ExpectedInstanceId != transfer.State.InstanceId) {
        ...
        async Task RenewRequiredAsync() {
            ...
            var result = await gateway.RenewRetentionAsync(connection.Manifest.Id, connection.Context, new(transfer.State.JobId!, until),
                cancellationToken);
        ...
        void ValidateSnapshot(RemoteTransferSnapshot snapshot) {
            if (snapshot is null || snapshot.ClientOperationId != operationId || snapshot.InstanceId != transfer.State.InstanceId
                ...
                || snapshot.Progress is { } progress && (!double.IsFinite(progress) || progress is < 0 or > 1)
        ...
        try {
            ...
            if (transfer.State.Phase == IntegrationTransferPhase.SubmissionUncertain) {
                ...
                if (snapshot is null && transfer.State.CancellationRequested) {
                    ...
                    var result = await gateway.CancelSubmissionAsync(cancel.Manifest.Id, cancel.Context, new(operationId),
                        cancellationToken);
                    if (result.InstanceId != transfer.State.InstanceId || result.ClientOperationId != operationId
                        || !result.PreventedAcceptance) {
            ...
            if (transfer.State.CancellationRequested) {
                ...
                snapshot = await gateway.CancelAsync(connection.Manifest.Id, connection.Context, new(transfer.State.JobId!),
                    cancellationToken);
                ValidateSnapshot(snapshot);
                transfer.ObserveCancellation(snapshot.InstanceId, snapshot.JobId, snapshot.Revision, snapshot.State,
                    snapshot.ManifestRevision);
                await PersistAsync();
                if (transfer.State.Phase != IntegrationTransferPhase.Cancelled) {
                    throw new JobRetryLaterException("Waiting for the executor to stop the accepted operation.",
                        PollDelay(snapshot.NextPollAfter));
                }
            ...
            if (transfer.Phase.AwaitsRemoteExecution) {
                if (snapshot is null) {
                    ...
                    snapshot = await gateway.GetJobAsync(connection.Manifest.Id, connection.Context, new(transfer.State.JobId!),
                        cancellationToken);
                }
                ...
                if (transfer.State.Phase == IntegrationTransferPhase.AwaitingRemote) {
                    throw new JobRetryLaterException("Waiting for the executor to finish the accepted item.",
                        PollDelay(snapshot.NextPollAfter));
                }

                if (RemoteJobStateDefinition.For(snapshot.State) is { IsTerminal: true, RetainsManifest: false }) {
                    return;
                }
                ...
                if (snapshot.State == RemoteJobState.Partial) {
                    ...
                    throw new JobRetryLaterException("Partial outputs require review; preserving the executor's retention lease.",
                        TimeSpan.FromMinutes(5));
                }

                if (snapshot.ArtifactsExpired) {
                    if (transfer.State.Phase == IntegrationTransferPhase.AwaitingArtifacts) {
                        transfer.HoldUnavailableRemoteOutputs();
                    }

                    await PersistAsync(
                        "The executor's outputs expired before local verification. Restore those exact outputs before retrying.");
            ...
            if (transfer.Phase.AwaitsManifest) {
                ...
                var manifest = await manifests.ReadAsync(connection.Manifest.Id, connection.Context, transfer.State.JobId!,
                    transfer.State.ManifestRevision!, cancellationToken);
                if (!SupportsOutputs(manifest.Artifacts, work.Plan, intent)) {
                    ...
                    throw new JobRetryLaterException("Outputs require review; preserving the executor's retention lease.",
                        TimeSpan.FromMinutes(5));
                }
            ...
            if (transfer.State.Phase == IntegrationTransferPhase.Transferring) {
                foreach (var artifact in transfer.State.Artifacts!) {
                    if (transfer.State.VerifiedArtifactIds?.Contains(artifact.Id) == true) {
                        continue;
                    }
                    ...
                    if (verified is null) {
                        ...
                        if (delivery.ByteSize != artifact.SizeBytes
                            || !string.Equals(delivery.Sha256, artifact.Sha256, StringComparison.OrdinalIgnoreCase)) {
                        ...
                        verified = await bytes.TransferAsync(new(operationId, artifact.Id, connection.Context.BaseUrl,
                            ...
                            Math.Min(intent.MaximumBytes, IntegrationImportPolicy.For(work.Plan.EntityKind).Content.MaximumBytes)),
                            cancellationToken);
                    }
            ...
            if (transfer.State.Phase == IntegrationTransferPhase.Importing) {
                foreach (var artifact in transfer.State.Artifacts!) {
                    if (transfer.State.Imports?.Any(imported => imported.ArtifactId == artifact.Id) == true) {
                        continue;
                    }
                    ...
                    var verified = await bytes.ReadVerifiedAsync(operationId, artifact.Id, Path.GetFileName(artifact.RelativePath),
                        artifact.SizeBytes, artifact.Sha256, cancellationToken)
                    ...
                    await verifier.VerifyAsync(verified,
                        work.Plan.EntityKind == EntityKind.Gallery ? EntityKind.Image : work.Plan.EntityKind, cancellationToken);
                    ...
                    transfer.RecordImported(new(artifact.Id, artifact.Sha256,
                        imported.Entities.Select(entity => entity.Id).Distinct().ToArray()));
    ...
    private static bool SupportsOutputs(IReadOnlyList<IntegrationArtifact> artifacts, IntegrationTransferPlan plan,
        SubmitTransferInput intent) {
        if (plan.EntityKind != EntityKind.Gallery) {
            return SupportsSingleArtifact(artifacts, plan, intent);
        }

        if (intent.ItemIds.Count != 1) {
            return false;
        }

        try {
            _ = new IntegrationGalleryOutputSet(artifacts, intent.ItemIds[0], intent.MaximumBytes);
            return true;
    ...
    private static bool SupportsSingleArtifact(IReadOnlyList<IntegrationArtifact> artifacts, IntegrationTransferPlan plan,
        SubmitTransferInput intent) =>
