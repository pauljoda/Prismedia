...
public sealed class ConnectionService(IIntegrationConnectionStore store, IIntegrationPluginGateway plugins) {
    ...
    public async Task<ConnectionResponse> UpdateAsync(Guid id, UpdateConnectionRequest request, CancellationToken cancellationToken) {
        ...
        if (connection.State.Revision != request.ExpectedRevision) {
    ...
    public async Task<ConnectionResponse> ProbeAsync(Guid id, CancellationToken cancellationToken) {
        ...
        if (!connection.State.Enabled) {
            throw new ArgumentException("Enable the connection before testing it.");
        }
        ...
        try {
            ...
            var result = await plugins.ProbeAsync(manifest.Id,
                new IntegrationConnectionContext(id, connection.State.BaseUrl,
                    connection.State.HasPersistentRemoteIdentity ? connection.State.RemoteInstanceId : null,
                    connection.State.Settings, auth),
                cancellationToken);
    ...
    private async Task<PluginManifest> RequirePluginAsync(string pluginId, CancellationToken cancellationToken) =>
        await plugins.FindAsync(pluginId, cancellationToken)
        ?? throw new IntegrationInvocationException("Install and enable a compatible integration plugin first.");
    ...
    private static void Validate(PluginManifest manifest, IReadOnlyList<PluginCapability> capabilities,
        ...
        if (manifest.Integration is null || capabilities is null
            || capabilities.Any(kind => !manifest.Integration.Capabilities.Any(item => item.Kind == kind))) {
        ...
        if (settings.Any(pair => !fields.Any(field => field.Key == pair.Key))) {
            throw new ArgumentException("The plugin does not declare one of these settings.");
        }
        ...
        foreach (var field in fields) {
            ...
            if (field.Type == PluginSearchFieldType.Number
    ...
    private static ConnectionResponse ToResponse(StoredIntegrationConnection stored) {
        ...
        return new(value.Id, value.PluginId, value.Name, value.BaseUrl, value.Enabled, value.EnabledCapabilities,
            ...
            value.HasPersistentRemoteIdentity, value.EffectiveCapabilities.Select(item =>
                new PluginIntegrationCapability(item.Kind, item.Operations, item.EntityKinds)).ToArray(),
            value.LastCheckedAt, value.LastError);
    }
