...
public sealed class ExternalLibraryService(
    ...
    public Task<IReadOnlyList<ExternalLibraryMount>> ListAsync(Guid connectionId, CancellationToken token) =>
        mounts.ListAsync(connectionId, token);
    ...
    public async Task<IReadOnlyList<ExternalLibraryMount>> ListSuitableAsync(Guid connectionId, EntityKind kind, CancellationToken token) {
        if (!Enum.IsDefined(kind)) {
            throw new ArgumentException("Choose a supported library kind.");
        }
    ...
    public async Task<ExternalLibraryMount> CreateAsync(Guid connectionId, CreateExternalLibraryMountRequest request,
        ...
        var authorized = await access.RequireAsync(connectionId, PluginCapability.ConnectedLibrary,
            IntegrationOperation.ListLibraries, token);
    ...
    public async Task<ExternalLibraryMount> AttachAsync(Guid connectionId, AttachExistingExternalLibraryMountRequest request,
        CancellationToken token) {
