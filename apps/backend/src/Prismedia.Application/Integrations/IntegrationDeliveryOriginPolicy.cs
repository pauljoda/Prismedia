...
public static class IntegrationDeliveryOriginPolicy {
    ...
    public static bool HasValidDeclaration(PluginIntegrationDefinition definition) {
        ...
        if (origins.Count == 0 && suffixes.Count == 0) {
            return true;
        }

        if (origins.Count + suffixes.Count > 8
            || definition.Capabilities?.Any(capability => capability?.Kind == PluginCapability.AcquisitionSource
                && capability.Operations?.Contains(IntegrationOperation.Resolve) == true) != true) {
            return false;
        }

        var identities = new HashSet<string>(StringComparer.Ordinal);
        foreach (var value in origins) {
            if (!TryOrigin(value, out var origin) || !identities.Add(OriginKey(origin))) {
                return false;
            }
        }
        ...
        foreach (var value in suffixes) {
            if (!TryHostSuffix(value) || !hosts.Add(value)) {
                return false;
            }
        }
    ...
    /// <summary>Returns the single origin the HTTP transport must enforce, including on redirects. Foreign origins never receive
    /// plugin-supplied headers.</summary>
    public static string RequireAllowedOrigin(PluginIntegrationDefinition? definition, string baseUrl, HttpArtifactDelivery delivery) {
        if (!TryAddress(baseUrl, out var connection) || !TryAddress(delivery.Url, out var target)
            || delivery.Headers is null) {
            throw Rejected();
        }

        if (OriginKey(connection) == OriginKey(target)) {
            return connection.GetLeftPart(UriPartial.Authority);
        }
    ...
    private static bool TryOrigin(string? value, out Uri origin) {
        if (!TryAddress(value, out origin) || value!.Length > 2048 || !value.StartsWith("https://", StringComparison.OrdinalIgnoreCase)
            || origin.Scheme != Uri.UriSchemeHttps || value.Contains('?')) {
            return false;
        }
