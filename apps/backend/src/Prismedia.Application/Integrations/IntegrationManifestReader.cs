...
public sealed class IntegrationManifestReader(IIntegrationTransferGateway gateway) {
    ...
    public async Task<IntegrationArtifactManifest> ReadAsync(string pluginId, IntegrationConnectionContext connection,
        ...
        do {
            ...
            if (page.JobId != jobId || page.Revision != revision || !page.Sealed || page.ArtifactCount is < 1 or > IntegrationArtifactManifest.MaximumArtifacts
            ...
            if (cursor is not null && !cursors.Add(cursor)) throw new IntegrationInvocationException("The artifact manifest repeated a continuation cursor.");
        } while (cursor is not null);
        try { return new(jobId, revision, true, count!.Value, artifacts); }
        catch (ArgumentException) { throw new IntegrationInvocationException("The sealed output manifest contains invalid or incomplete artifact evidence."); }
    }
}
