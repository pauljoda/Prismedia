...
public sealed class CatalogDiscoveryService(IntegrationConnectionAccess access, IIntegrationDiscoveryGateway gateway,
    ...
    public async Task<DiscoveryPageResponse> BrowseAsync(Guid connectionId, BrowseConnectionRequest request,
        ...
        var authorized = await access.RequireAsync(connectionId, PluginCapability.CatalogDiscovery, operation,
        ...
        if (!string.IsNullOrWhiteSpace(request.Container)) {
            ...
            if (selection.EntityKind != request.EntityKind) {
                throw new ArgumentException("The selected catalog belongs to another media type.");
            }
    ...
    /// <summary>Resolves a protected selection for server-side acquisition. Lending, checkout, and sample offers cannot satisfy a
    /// full-content request.</summary>
    public async Task<ResolvedSourceOffer> ResolveAsync(Guid connectionId, string selectionToken, string offerId,
        CancellationToken cancellationToken) {
    ...
    public async Task<ResolvedSourceOffer> ResolveSelectionAsync(Guid connectionId, SourceSelection selection, string offerId,
        CancellationToken cancellationToken) {
        if (string.IsNullOrWhiteSpace(offerId) || offerId.Length > 2048) {
            throw new ArgumentException("Select a valid acquisition offer.");
        }
        ...
        ValidatePage(new("Resolved publication", [new(selection, false, resolved.Publication, [resolved.Offer])]),
            new(selection.EntityKind, Limit: 1));
        _ = IntegrationDeliveryOriginPolicy.RequireAllowedOrigin(authorized.Manifest.Integration, authorized.Context.BaseUrl,
            resolved.Delivery);
        return resolved;
    }
    ...
    internal static void ValidatePage(CatalogPage page, BrowseConnectionRequest request) {
        if (string.IsNullOrWhiteSpace(page.Title) || page.Title.Length > 512 || page.NextCursor?.Length > 8192
            || page.Items is null || page.Items.Count > request.Limit) {
            throw InvalidPage();
        }
        ...
        foreach (var item in page.Items) {
            if (item?.Selection is null || item.Selection.EntityKind != request.EntityKind
                ...
                || item.Offers.Count > 32 || item.IsContainer && item.Offers.Count > 0) {
                throw InvalidPage();
            }
            ...
            if (string.IsNullOrWhiteSpace(publication.Title) || publication.Title.Length > 512
                ...
                || new[] { publication.Language, publication.Publisher, publication.EditionLabel, publication.IssueLabel }
                    .Any(value => value?.Length > 512)
                || !ValidAttribution(publication.Attribution)) {
                throw InvalidPage();
            }
            ...
            foreach (var offer in item.Offers) {
                if (offer is null || string.IsNullOrWhiteSpace(offer.Id) || offer.Id.Length > 2048 || !offers.Add(offer.Id)
                    ...
                    || offer.MediaType?.Length > 256 || offer.ByteSize is < 0) {
                    throw InvalidPage();
                }
            }
        }
