...
public interface IIntegrationArtifactTransfer {
    ...
    /// <summary>Reopens already verified bytes using persisted evidence only. Missing staging never initiates another remote download.</summary>
    Task<VerifiedIntegrationArtifact?> ReadVerifiedAsync(Guid operationId, string artifactId, string fileName, long sizeBytes, string sha256, CancellationToken cancellationToken);
}
