...
public sealed class ManagedControlService(IManagedControlStore store, IntegrationConnectionAccess access,
    ...
    private async Task<ManagedControlPreview> PreviewAsync(Guid connectionId, OwnedManagedControlScope owned, CancellationToken token) {
        var connection = await access.RequireAsync(connectionId, PluginCapability.ExternalManager, IntegrationOperation.ReconcileManaged,
            owned.Scope.Item.EntityKind, token);
    ...
    public async Task<ManagedControlActionResponse> CreateAsync(Guid connectionId, Guid holdingId, CreateManagedControlRequest request,
    ...
    internal async Task<ManagedControlActionResponse> CreateAsync(Guid connectionId, Guid holdingId,
        ...
        if (await store.FindAsync(request.OperationId, token) is { } existing) {
            if (existing.Operation.State.ConnectionId != connectionId || existing.Operation.State.HoldingId != holdingId
                || existing.Plan.RequestFingerprint != fingerprint) {
        ...
        if (owned.Fingerprint != request.ScopeFingerprint
            || !owned.Scope.Targets.Select(target => target.RemoteId).ToHashSet(StringComparer.Ordinal)
                .SetEquals(request.ExpectedMonitoring.Keys)
        ...
        await access.RequireAsync(connectionId, PluginCapability.ExternalManager, IntegrationOperation.ReconcileManaged,
            owned.Scope.Item.EntityKind, token);
        var configure = request.Changes.ProfileId is not null || request.Changes.Monitored is not null;
        if (configure) {
            await access.RequireAsync(connectionId, PluginCapability.ExternalManager, IntegrationOperation.ConfigureManaged,
                owned.Scope.Item.EntityKind, token);
        }
    ...
    public Task<ManagedControlActionResponse> CancelAsync(Guid connectionId, Guid holdingId, Guid id, long revision,
        CancellationToken token) =>
    ...
    public Task<ManagedControlActionResponse> CloseUnverifiedAsync(Guid connectionId, Guid holdingId, Guid id, long revision,
        CancellationToken token) =>
    ...
    private async Task<ManagedControlActionResponse> ChangeAsync(Guid connectionId, Guid holdingId, Guid id, long revision,
        ...
        if (work.Operation.State.Revision != revision) {
            throw new ManagedControlConflictException("This action changed. Reload its progress first.");
        }

        try {
            change(work.Operation);
        } catch (InvalidOperationException error) {
            throw new ManagedControlConflictException(error.Message);
        }
    ...
    private static void Validate(CreateManagedControlRequest request) {
        if (request.OperationId == Guid.Empty || request.ScopeFingerprint is not { Length: 64 }
            ...
            || request.ExpectedMonitoring is not { Count: <= 10000 }
            || request.ExpectedMonitoring.Keys.Any(key => string.IsNullOrWhiteSpace(key) || key.Length > 512)
            || request.Changes is null
            || request.Changes.ProfileId is { } profile && (string.IsNullOrWhiteSpace(profile) || profile.Length > 512)
