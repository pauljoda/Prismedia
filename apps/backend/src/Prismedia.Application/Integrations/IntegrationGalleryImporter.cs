...
public sealed class IntegrationGalleryImporter(IIntegrationArtifactTransfer bytes, IIntegrationMediaVerifier verifier,
    ...
    public async Task<IReadOnlyList<IntegrationArtifactImport>> ImportAsync(StoredIntegrationTransfer work, JobContext context, CancellationToken token) {
        ...
        foreach (var artifact in outputs.Artifacts) {
            var file = await bytes.ReadVerifiedAsync(operationId, artifact.Id, Path.GetFileName(artifact.RelativePath), artifact.SizeBytes, artifact.Sha256, token)
                ?? throw new InvalidDataException("Verified gallery staging is missing.");
        ...
        var placed = await placement.PlaceAsync(new(operationId, work.Plan, root, outputs, verified), token);
