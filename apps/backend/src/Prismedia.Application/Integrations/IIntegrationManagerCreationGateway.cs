...
public interface IIntegrationManagerCreationGateway {
    ...
    Task<ManagedLookupResult> LookupAsync(string pluginId, IntegrationConnectionContext connection, ManagedLookupInput input, CancellationToken token);
    /// <summary>Ensures initial unmonitored holding existence once. Persist ownership and a dispatch fence before calling.</summary>
    Task<EnsureManagedResult> EnsureAsync(string pluginId, IntegrationConnectionContext connection, EnsureManagedInput input, CancellationToken token);
}
