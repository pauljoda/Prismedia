...
/// <summary>Accepts explicit wanted-movie fulfillment through a chosen external manager and mapped library.</summary>
public sealed class ManagedRequestService(IManagedRequestStore store, IntegrationConnectionAccess access,
    ...
    public async Task<ManagedRequestPreview> PreviewAsync(Guid connectionId, PreviewManagedRequestInput input, CancellationToken token) {
        var target = await store.RequireTargetAsync(connectionId, input.EntityId, input.LibraryRootId, token);
    ...
    public async Task<ManagedRequestResponse> CreateAsync(Guid connectionId, CreateManagedRequestInput input, CancellationToken token) {
        ...
        var preview = await PreviewAsync(connectionId, new(input.EntityId, input.LibraryRootId), token);
        ...
        if (preview.Existing is { } holding && holding.Item.ProfileId != input.ProfileId)
            throw new ManagedRequestConflictException("This movie already exists with another profile. Review and use its current profile before changing it through linked controls.");
    ...
    private static void Validate(CreateManagedRequestInput input) {
        if (input.OperationId == Guid.Empty || input.EntityId == Guid.Empty || input.LibraryRootId == Guid.Empty
            || input.ReviewedWork is not { EntityKind: EntityKind.Movie, ExternalIds.Count: > 0 and <= 64 }
            ...
            throw new ArgumentException("Select a reviewed wanted movie, mapped root, and external profile.");
    }
