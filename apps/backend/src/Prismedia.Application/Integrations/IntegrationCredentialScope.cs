...
public static class IntegrationCredentialScope {
    #region Actions - Scoping

    /// <summary>Copies only exactly declared keys and verifies required values. Undeclared saved credentials remain available for
    /// recovery but never enter this invocation.</summary>
    public static IReadOnlyDictionary<string, string> ForManifest(PluginManifest manifest, IReadOnlyDictionary<string, string> saved) {
        ...
        var scoped = saved.Where(pair => keys.Contains(pair.Key))
            .ToDictionary(pair => pair.Key, pair => pair.Value, StringComparer.Ordinal);
        if (manifest.Auth.Any(field => field.Required
            && (!scoped.TryGetValue(field.Key, out var value) || string.IsNullOrWhiteSpace(value)))) {
