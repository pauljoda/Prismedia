...
public interface IExternalLibraryMountStore {
    ...
    /// <summary>Atomically makes one reviewed existing library an external boundary without changing its contents or configuration.</summary>
    Task<ExternalLibraryMount> AttachAsync(Guid connectionId, long expectedRevision, AttachExistingExternalLibraryMountRequest request, CancellationToken token);
    /// <summary>Attaches an existing root and reports whether this call created the immutable boundary.</summary>
    Task<ExternalLibraryMountAttachment> AttachWithResultAsync(Guid connectionId, long expectedRevision, AttachExistingExternalLibraryMountRequest request, CancellationToken token);
