...
public sealed class ExecutorAcquisitionService(IntegrationConnectionAccess access, IIntegrationTransferGateway gateway,
    ...
    private static readonly IntegrationOperation[] RequiredOperations = [IntegrationOperation.Submit, IntegrationOperation.FindSubmission,
        ...
        IntegrationOperation.RenewRetention, IntegrationOperation.Acknowledge, IntegrationOperation.Cancel,
    ...
    /// <summary>Returns finite choices from an executor that provides durable identity, submission recovery, retained outputs, and
    /// receipts.</summary>
    public async Task<ExecutorInspectionResponse> InspectAsync(Guid connectionId, InspectExecutorRequest request,
        CancellationToken cancellationToken) {
        RequireImportKind(request.EntityKind);
        RequireSourceUrl(request.Url);
        var connection = await access.RequireAsync(connectionId, PluginCapability.CatalogDiscovery, IntegrationOperation.Inspect,
            request.EntityKind, cancellationToken);
        ...
        if (inspection.ExpiresAt > DateTimeOffset.UtcNow.AddHours(1)) {
            inspection = inspection with { ExpiresAt = DateTimeOffset.UtcNow.AddHours(1) };
        }

        var token = selections.Protect(connectionId, new(connection.Connection.State.RemoteInstanceId!,
            connection.Connection.State.Revision, request.EntityKind, inspection));
    ...
    public async Task<IntegrationTransferResponse> AcquireAsync(Guid connectionId, AcquireExecutorItemRequest request,
        CancellationToken cancellationToken) {
        if (request.OperationId == Guid.Empty || request.LibraryRootId == Guid.Empty || string.IsNullOrWhiteSpace(request.ItemId)
            || request.ItemId.Length > 2048 || string.IsNullOrWhiteSpace(request.SelectionToken)
            || request.SelectionToken.Length > 262144) {
        ...
        var connection = await access.RequireAsync(connectionId, PluginCapability.TransferExecutor, IntegrationOperation.Submit,
            selection.EntityKind, cancellationToken);
        RequireExecutor(connection, selection.EntityKind);
        if (connection.Connection.State.RemoteInstanceId != selection.InstanceId
            || connection.Connection.State.Revision != selection.ConnectionRevision) {
        ...
        await store.CreateAsync(IntegrationTransfer.Create(request.OperationId, connectionId, selection.InstanceId),
    ...
    private static void RequireExecutor(AuthorizedIntegrationConnection connection, EntityKind kind) {
        if (!connection.Connection.State.HasPersistentRemoteIdentity
            || string.IsNullOrWhiteSpace(connection.Connection.State.RemoteInstanceId)
            ...
            throw new ArgumentException(
                "This executor must support persistent identity, submission recovery, retained artifacts, and import receipts.");
        }
    }
    ...
    private static void ValidateInspection(TransferInspection inspection, EntityKind kind) {
        ...
        if (inspection is null || string.IsNullOrWhiteSpace(inspection.SelectionId) || inspection.SelectionId.Length > 512
            || string.IsNullOrWhiteSpace(inspection.Revision) || inspection.Revision.Length > 512
            || inspection.ExpiresAt <= DateTimeOffset.UtcNow
            ...
            || inspection.Items.Any(item => string.IsNullOrWhiteSpace(item.Id) || item.Id.Length > 2048
                || string.IsNullOrWhiteSpace(item.Title) || item.Title.Length > 512 || item.EntityKind != kind)
            || inspection.Warnings is null || inspection.Warnings.Count > 20
            || inspection.Warnings.Any(message => message is null || message.Length > 1024)) {
    ...
    private static void RequireImportKind(EntityKind kind) {
        if (kind is not (EntityKind.Book or EntityKind.ComicInstallment or EntityKind.Image or EntityKind.Gallery)) {
            throw new ArgumentException("Choose a book, comic installment, image, or gallery for URL acquisition.");
        }
    }
