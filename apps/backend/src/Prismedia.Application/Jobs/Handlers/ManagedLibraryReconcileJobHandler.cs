...
public sealed class ManagedLibraryReconcileJobHandler(ManagedTrackingService service, ManagedRequestProcessor requests, ManagedReleaseService releases) : IJobHandler {
    /// <inheritdoc />
    public async Task HandleAsync(JobContext context, CancellationToken cancellationToken) {
        ...
        if (await releases.ProcessAsync(id, cancellationToken)) return;
