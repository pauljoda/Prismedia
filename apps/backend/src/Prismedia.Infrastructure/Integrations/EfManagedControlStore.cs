...
public sealed class EfManagedControlStore(PrismediaDbContext db, IManagedTrackingStore tracking,
    IJobQueueService queue) : IManagedControlStore {
    ...
    private async Task<ManagedTrackingResponse> RequireHoldingAsync(Guid connectionId, Guid holdingId, CancellationToken token) {
        ...
        if (holding is null || holding.ConnectionId != connectionId || !ManagedTrackingStatusDefinition.For(holding.Status).AcceptsControls
            ...
            throw new ManagedControlConflictException(
                "Refresh and verify this holding's tracked associations before changing its manager settings.");
        }
        ...
        if (entityIds.Length != holding.Targets.Count
            || holding.Targets.Select(binding => binding.Target.RemoteTargetId).Distinct(StringComparer.Ordinal)
                .Count() != holding.Targets.Count
            || sourceTargets.Select(binding => binding.Target.RemoteTargetId).Distinct(StringComparer.Ordinal)
                .Count() != sourceTargets.Length
    ...
    private async Task ValidateScopeAsync(ManagedControlOperation operation, ManagedControlPlan plan, CancellationToken token) {
        ...
        await db.ManagedHoldings.FromSqlInterpolated($"SELECT * FROM managed_holdings WHERE id = {operation.State.HoldingId} FOR UPDATE")
        ...
        if (current.Fingerprint != plan.Request.ScopeFingerprint) {
            throw new ManagedControlConflictException(
                "The reviewed target associations changed. Refresh the holding before creating another action.");
        }
    }
    ...
    /// <inheritdoc />
    public async Task<StoredManagedControl> CreateAsync(ManagedControlOperation operation, ManagedControlPlan plan,
        ...
        if (operation.State.Revision != 1 || plan.Request.OperationId != operation.State.OperationId
            || plan.RequestFingerprint != ManagedControlIdentity.RequestFingerprint(plan.Request)) {
            throw new ArgumentException("Invalid manager action intent.");
        }
    ...
    /// <inheritdoc />
    public async Task SaveAsync(ManagedControlOperation operation, long expectedRevision, string? problem, bool beforeDispatch,
        CancellationToken token) {
        var stored = await FindAsync(operation.State.OperationId, token)
            ?? throw new ManagedControlConflictException("The manager action no longer exists.");
        if (stored.Operation.State.ConnectionId != operation.State.ConnectionId
            || stored.Operation.State.HoldingId != operation.State.HoldingId
            || stored.Operation.State.Revision != expectedRevision || operation.State.Revision != expectedRevision + 1) {
            throw Conflict();
        }

        await using var transaction = await db.Database.BeginTransactionAsync(token);
        if (beforeDispatch) {
            await ValidateScopeAsync(operation, stored.Plan, token);
        }
    ...
    private static StoredManagedControl Map(ManagedControlRow row) {
        var state = JsonSerializer.Deserialize<ManagedControlOperationState>(row.StateJson, Json)
            ?? throw new InvalidDataException("Invalid manager action state.");
        var plan = JsonSerializer.Deserialize<ManagedControlPlan>(row.PlanJson, Json)
            ?? throw new InvalidDataException("Invalid manager action intent.");
        if (state.OperationId != row.Id || state.HoldingId != row.HoldingId || state.ConnectionId != row.ConnectionId
    ...
    private static StoredManagedControl Same(StoredManagedControl existing, ManagedControlOperation operation, ManagedControlPlan plan) {
        if (existing.Operation.State.ConnectionId != operation.State.ConnectionId
            || existing.Operation.State.HoldingId != operation.State.HoldingId
            || existing.Plan.RequestFingerprint != plan.RequestFingerprint) {
