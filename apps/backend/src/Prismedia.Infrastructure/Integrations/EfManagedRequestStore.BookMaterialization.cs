...
/// <summary>Attaches verified ebook and audiobook rendition files to an accepted Book request.</summary>
public sealed partial class EfManagedRequestStore {
    #region Actions - Book Materialization
    ...
    private async Task<ManagedRequestMaterialization> MaterializeBookAsync(
        ...
        if (snapshot.Files.Count == 0) {
            return new(false, "Waiting for the manager to import this Book rendition.");
        }
        ...
        if (!await lifecycle.ExecuteManyAsync(lifecycleIds, async ct => {
            ...
            if (detail is null) {
                db.BookDetails.Add(new() { EntityId = book.Id, BookType = BookType.Novel, Format = format });
            } else if (rendition == BookRendition.Ebook) {
            ...
            for (var index = 0; index < parts.Count; index++) {
                ...
                if (rendition == BookRendition.Audiobook && adopted is null) {
                    ...
                    if (existingFolder is null && index == 0) {
    ...
    #region Actions - Audiobook Validation
    ...
    private static void RequireCompleteAudiobookFolder(IReadOnlyList<string> paths, string libraryPath, CancellationToken token) {
        ...
        try {
            ...
            throw new ArgumentException(
                "The mapped audiobook folder could not be inspected. Review its files before completing this request.", error);
        }
    }
