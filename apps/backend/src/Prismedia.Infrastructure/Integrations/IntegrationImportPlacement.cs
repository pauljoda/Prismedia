...
public sealed class IntegrationImportPlacement(ILibraryFileMutationGuard mutations) : IIntegrationImportPlacement {
    ...
    public async Task<VerifiedIntegrationArtifact?> ReadPlacedAsync(Guid operationId, IntegrationTransferPlan plan, LibraryRootData root,
        ...
        var recovered = new VerifiedIntegrationArtifact(artifact.Id, target, artifact.SizeBytes,
            artifact.Sha256.ToLowerInvariant(), fileName);
    ...
    public async Task<string> PlaceAsync(Guid operationId, IntegrationTransferPlan plan, LibraryRootData root,
        ...
        var target = Path.Combine(rootPath,
            IntegrationPublicationNames.FileName(operationId, plan.Title, artifact.ArtifactId, artifact.FileName));
        var pending = Path.Combine(rootPath, $".prismedia-{operationId:N}-{artifactKey}.part");
        RejectLink(target);
        RejectLink(pending);
        if (File.Exists(target)) {
            await VerifyAsync(target, artifact, cancellationToken);
            return target;
        }
