...
public sealed class EfIntegrationTransferStore(PrismediaDbContext db, TransferPlanProtector protector,
    ...
    public async Task<StoredIntegrationTransfer?> FindAsync(Guid operationId, CancellationToken cancellationToken) =>
        await db.IntegrationTransfers.AsNoTracking().SingleOrDefaultAsync(row => row.Id == operationId, cancellationToken) is { } row
            ? Map(row)
            : null;
    ...
    /// <inheritdoc />
    public async Task<StoredIntegrationTransfer> CreateAsync(IntegrationTransfer transfer, IntegrationTransferPlan plan,
        CancellationToken cancellationToken) {
        Validate(transfer, plan);
        if (await FindAsync(transfer.State.OperationId, cancellationToken) is { } existing) {
            return SameIntent(existing, transfer, plan);
        }
        ...
        var row = new IntegrationTransferRow {
            ...
            ProtectedPlan = protector.Protect(transfer.State.ConnectionId, transfer.State.OperationId,
        ...
        try {
            ...
            if (await FindAsync(transfer.State.OperationId, cancellationToken) is { } concurrent) {
                return SameIntent(concurrent, transfer, plan);
            }
    ...
    public async Task SaveAsync(IntegrationTransfer transfer, long expectedRevision, string? error, CancellationToken cancellationToken) {
        ...
        if (row.Revision != expectedRevision || transfer.State.Revision != expectedRevision + 1
            || row.ConnectionId != transfer.State.ConnectionId) {
            throw new IntegrationTransferConflictException(
                "This transfer changed while the operation was running. Reload it before retrying.");
        }
        ...
        if (transfer.Phase.IsTerminal) {
            row.ActiveOwnershipKey = null;
        }

        try {
            await db.SaveChangesAsync(cancellationToken);
        } catch (DbUpdateConcurrencyException) {
            throw new IntegrationTransferConflictException("This transfer changed while saving its progress.");
        }
    }
    ...
    public async Task RecordErrorAsync(Guid operationId, long expectedRevision, string error, CancellationToken cancellationToken) {
        var row = await db.IntegrationTransfers.SingleOrDefaultAsync(item => item.Id == operationId && item.Revision == expectedRevision,
            cancellationToken);
        if (row is null) {
            return;
        }
        ...
        try {
            ...
            /* A newer progress revision owns the status. */
        }
    }
    ...
    public async Task SaveAndEnqueueAsync(IntegrationTransfer transfer, long expectedRevision, CancellationToken cancellationToken) {
        ...
        if (transfer.State.Phase != IntegrationTransferPhase.Cancelled) {
            await EnqueueRetryAsync(transfer.State.OperationId, cancellationToken);
        }
    ...
    private StoredIntegrationTransfer Map(IntegrationTransferRow row) {
        ...
        if (state.OperationId != row.Id || state.ConnectionId != row.ConnectionId || state.Revision != row.Revision
        ...
        var plan = JsonSerializer.Deserialize<IntegrationTransferPlan>(
            protector.Unprotect(row.ConnectionId, row.Id, row.ProtectedPlan), Json)
    ...
    private static StoredIntegrationTransfer SameIntent(StoredIntegrationTransfer existing, IntegrationTransfer transfer,
        IntegrationTransferPlan plan) {
        if (existing.Transfer.State.ConnectionId != transfer.State.ConnectionId
    ...
    private static void Validate(IntegrationTransfer transfer, IntegrationTransferPlan plan) {
        if (transfer.State.Revision != 1 || string.IsNullOrWhiteSpace(plan.Title) || plan.Title.Length > 512
            || !Enum.IsDefined(plan.EntityKind)
            || plan.LibraryRootId == Guid.Empty || string.IsNullOrWhiteSpace(plan.LibraryPath)
            || !Path.IsPathFullyQualified(plan.LibraryPath)
