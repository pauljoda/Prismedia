...
/// <summary>Bounded HTTP byte retrieval. A private receipt survives restarts; media validation and library import are separate steps.</summary>
public sealed class HttpIntegrationArtifactTransfer(IntegrationArtifactStorageOptions options, HttpClient client) : IIntegrationArtifactTransfer {
    ...
    /// <inheritdoc />
    public async Task<VerifiedIntegrationArtifact> TransferAsync(IntegrationArtifactTransferRequest request, CancellationToken cancellationToken) {
        Validate(request);
        ...
        foreach (var path in new[] { finalPath, partialPath, receiptPath, lockPath, receiptPath + ".tmp" }) RejectLinks(path);
        ...
        if (File.Exists(receiptPath)) return await ReadVerifiedAsync(request, finalPath, receiptPath, cancellationToken);
        ...
        if (response.StatusCode == HttpStatusCode.OK) offset = 0; // The server may ignore Range; restart rather than append a full body.
        ...
        } else throw new HttpRequestException($"Artifact retrieval returned HTTP {(int)response.StatusCode}.");
        ...
        if (response.Content.Headers.ContentLength is { } responseSize && (responseSize > request.MaximumBytes - offset
            || delivery.ByteSize is { } length && responseSize != length - offset)) throw new InvalidDataException("The artifact response length does not match its declared limits.");
        await using (var output = new FileStream(partialPath, offset == 0 ? FileMode.Create : FileMode.Open, FileAccess.Write, FileShare.None, 1024 * 128, true)) {
        ...
        try { verified = await VerifyAsync(request, partialPath, cancellationToken); }
        catch (InvalidDataException) { File.Delete(partialPath); throw; }
        File.Move(partialPath, finalPath);
    ...
    private async Task<HttpResponseMessage> OpenAsync(Uri origin, Uri address, IReadOnlyDictionary<string, string> headers, long offset, CancellationToken cancellationToken) {
        for (var redirects = 0; redirects <= 5; redirects++) {
            ...
            using (response) {
                if (redirects == 5 || response.Headers.Location is not { } location) throw new InvalidDataException("The artifact redirect chain is invalid.");
    ...
    private static async Task<VerifiedIntegrationArtifact> VerifyAsync(IntegrationArtifactTransferRequest request, string path, CancellationToken cancellationToken) {
        RejectLinks(path);
