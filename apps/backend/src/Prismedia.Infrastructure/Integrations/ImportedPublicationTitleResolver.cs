...
/// <summary>
/// Uses the existing durable import journal, exact accepted path, and verified bytes rather than heuristically
/// stripping filename suffixes.
/// </summary>
public sealed class ImportedPublicationTitleResolver(IIntegrationTransferStore transfers) : IImportedPublicationTitleResolver {
    ...
    public async Task<string?> ResolveAsync(Guid libraryRootId, EntityKind kind, string sourcePath, CancellationToken cancellationToken) {
        ...
        if (kind == EntityKind.Image
            && await ResolveGalleryAsync(libraryRootId, Path.GetDirectoryName(sourcePath)!, sourcePath,
                cancellationToken) is { } memberTitle) {
            return memberTitle;
        }
    ...
    /// <inheritdoc />
    public async Task<ImportedComicPublication?> ResolveComicAsync(Guid libraryRootId, string sourcePath,
    ...
    private async Task<StoredIntegrationTransfer?> ResolveFileAsync(Guid libraryRootId, EntityKind kind, string sourcePath,
        CancellationToken cancellationToken) {
        if (IntegrationPublicationNames.CandidateOperation(sourcePath) is not { } id) {
            return null;
        }
        ...
        if (work is null || work.Plan.LibraryRootId != libraryRootId || work.Plan.EntityKind != kind
            || work.Transfer.State.Artifacts is not { Count: > 0 } artifacts) {
            return null;
        }
    ...
    private async Task<string?> ResolveGalleryAsync(Guid libraryRootId, string folder, string? memberPath, CancellationToken token) {
        if (IntegrationPublicationNames.CandidateFolderOperation(folder) is not { } id) {
            return null;
        }
        ...
        if (work is null || work.Plan.LibraryRootId != libraryRootId || work.Plan.EntityKind != EntityKind.Gallery
            || work.Plan.Executor is not { ItemIds.Count: 1 } intent || work.Transfer.State.Artifacts is not { } artifacts) {
            return null;
        }

        IntegrationGalleryOutputSet outputs;
        try {
            outputs = new(artifacts, intent.ItemIds[0], intent.MaximumBytes);
        } catch (ArgumentException) {
            return null;
        }

        var expectedFolder = Path.Combine(Path.GetFullPath(work.Plan.LibraryPath),
            IntegrationGalleryNames.FolderName(id, work.Plan.Title, outputs.GroupId));
        if (!FileSystemPathComparison.Equals(Path.GetFullPath(folder), expectedFolder)
            || outputs.Artifacts.Any(artifact => work.Transfer.State.VerifiedArtifactIds?.Contains(artifact.Id) != true)) {
            return null;
        }
        ...
        foreach (var artifact in outputs.Artifacts) {
            ...
            if (memberPath is not null && !FileSystemPathComparison.Equals(Path.GetFullPath(memberPath), expectedPath)) {
                continue;
            }

            if (!await MatchesAsync(expectedPath, artifact, token)) {
                return null;
            }

            if (memberPath is not null) {
                return Path.GetFileNameWithoutExtension(artifact.RelativePath);
            }
        }
