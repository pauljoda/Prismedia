...
public sealed class EfIntegrationArtifactStagingMaintenance(
    ...
    public async Task<int> SweepAsync(DateTimeOffset completedBefore, CancellationToken cancellationToken) {
        ...
        foreach (var directory in Directory.EnumerateDirectories(root)) {
            ...
            if (!Guid.TryParseExact(name, "N", out var operationId)
                || !Path.GetFullPath(directory).Equals(Path.Combine(root, operationId.ToString("N")),
                    FileSystemPathComparison.Comparison)) {
                continue;
            }

            try {
                if (await TryDeleteAsync(operationId, directory, completedBefore, cancellationToken)) {
                    deleted++;
                }
    ...
    private async Task<bool> TryDeleteAsync(Guid operationId, string directory, DateTimeOffset cutoff, CancellationToken token) {
        ...
        if (!HasCompleteImportEvidence(row, state)) {
            return false;
        }
        ...
        if (!HasOnlyAllowedFiles(directory, allowed)) {
            return false;
        }
        ...
        try {
            ...
            if (row.Revision != lockedRevision || !HasCompleteImportEvidence(row, state)
                ...
                || !HasOnlyAllowedFiles(directory, AllowedNames(state.Artifacts!))) {
                return false;
            }
    ...
    private static bool HasCompleteImportEvidence(IntegrationTransferRow row, IntegrationTransferState state) {
        if (row.Phase != IntegrationTransferPhase.Completed || state.OperationId != row.Id || state.ConnectionId != row.ConnectionId
            || state.Revision != row.Revision || state.Phase != IntegrationTransferPhase.Completed
            || state.ReceiptId is not { } receipt || receipt == Guid.Empty
            ...
            || imports.Select(item => item.ArtifactId).Distinct(StringComparer.Ordinal).Count() != imports.Count) {
            return false;
        }

        if (IntegrationTransferModeDefinition.For(state.Mode).AcknowledgesImports && state.ReceiptId is null) {
            return false;
        }
    ...
    #region Actions - Filesystem Safety
    ...
    private static bool HasOnlyAllowedFiles(string directory, HashSet<string> allowed) {
        ...
        foreach (var entry in Directory.EnumerateFileSystemEntries(directory)) {
            RejectLink(entry);
            if (Directory.Exists(entry) || !allowed.Contains(Path.GetFileName(entry))) {
                return false;
            }
        }
