...
/// <summary>Seals short-lived source navigation and selection tokens without exposing source locators or accepting client-authored URLs.</summary>
public sealed class DiscoveryTokenProtector(string dataDirectory) : IDiscoveryTokenProtector {
    ...
    public string ProtectSelection(Guid connectionId, SourceSelection selection) =>
        Protector(SelectionPurpose, connectionId).Protect(JsonSerializer.Serialize(selection, PluginProcessTransport.JsonOptions), Lifetime);

    /// <inheritdoc />
    public SourceSelection ReadSelection(Guid connectionId, string token) {
        try {
            return JsonSerializer.Deserialize<SourceSelection>(Read(Protector(SelectionPurpose, connectionId), token), PluginProcessTransport.JsonOptions)
                ?? throw InvalidToken();
        } catch (JsonException) { throw InvalidToken(); }
    }
    ...
    private static string Read(ITimeLimitedDataProtector protector, string token) {
        if (string.IsNullOrWhiteSpace(token) || token.Length > 32_768) throw InvalidToken();
        try { return protector.Unprotect(token); }
        catch (CryptographicException) { throw InvalidToken(); }
    }
    ...
    private static ArgumentException InvalidToken() => new("This catalog selection or page token is invalid or expired. Refresh the catalog and select it again.");
}
