...
public sealed class IntegrationMediaVerifier : IIntegrationMediaVerifier {
    ...
    public async Task VerifyAsync(VerifiedIntegrationArtifact artifact, EntityKind kind, CancellationToken cancellationToken) {
        ...
        try {
            ...
            if (Path.GetExtension(artifact.FileName).Equals(".pdf", StringComparison.OrdinalIgnoreCase)) {
                ...
                if (document.NumberOfPages is < 1 or > MaximumEntries) {
                    throw new InvalidDataException("The PDF has no readable pages or exceeds the page limit.");
                }
            ...
            if (kind == EntityKind.ComicInstallment) {
                ValidateComic(archive, cancellationToken);
            } else {
                await ValidateEpubAsync(archive, cancellationToken);
            }
    ...
    private static void ValidateImage(VerifiedIntegrationArtifact artifact, CancellationToken cancellationToken) {
        ...
        if (artifact.SizeBytes <= 0 || file.Length != artifact.SizeBytes
            || file.Length > IntegrationImportPolicy.For(EntityKind.Image).MaximumBytes) {
    ...
    #region Actions - Publications

    private static void ValidateArchive(ZipArchive archive) {
        if (archive.Entries.Count is < 1 or > MaximumEntries) {
            throw new InvalidDataException("The publication archive exceeds its entry limit.");
        }
        ...
        foreach (var entry in archive.Entries) {
            ...
            if (entry.Length > MaximumExpandedBytes - expanded) {
                throw new InvalidDataException("The publication archive exceeds its expanded byte limit.");
            }
    ...
    private static async Task ValidateEpubAsync(ZipArchive archive, CancellationToken cancellationToken) {
        ...
        if (mimetype.Length > 128) {
            throw new InvalidDataException("The EPUB publication type marker is invalid.");
        }

        using (var reader = new StreamReader(mimetype.Open(), Encoding.UTF8)) {
            if ((await reader.ReadToEndAsync(cancellationToken)).Trim() != "application/epub+zip") {
                throw new InvalidDataException("The downloaded archive is not an EPUB publication.");
            }
        }
        ...
        if (packagePath is null) {
            throw new InvalidDataException("The EPUB does not identify a package document.");
        }
        ...
        var items = package.Root?.Element(PackageNamespace + "manifest")?.Elements(PackageNamespace + "item")
            ...
            .ToDictionary(item => item.Attribute("id")!.Value, item => item.Attribute("href")?.Value, StringComparer.Ordinal)
        ...
        if (spine.Length == 0) {
            throw new InvalidDataException("The EPUB has no reading order.");
        }
    ...
    private static async Task<XDocument> ReadXmlAsync(ZipArchiveEntry? entry, CancellationToken cancellationToken) {
        if (entry is null || entry.Length > MaximumXmlBytes) {
            throw new InvalidDataException("The EPUB contains a missing or oversized package document.");
        }
    ...
    #region Actions - Comics

    private static void ValidateComic(ZipArchive archive, CancellationToken cancellationToken) {
        var pages = archive.Entries.Where(entry => Path.GetExtension(entry.FullName).ToLowerInvariant()
            is ".jpg" or ".jpeg" or ".png" or ".webp" or ".gif" or ".bmp" or ".avif").ToArray();
        if (pages.Length == 0) {
            throw new InvalidDataException("The comic archive has no readable image pages.");
        }

        foreach (var page in pages) {
            cancellationToken.ThrowIfCancellationRequested();
            if (page.Length > IntegrationImportPolicy.For(EntityKind.Image).MaximumBytes) {
                throw new InvalidDataException("A comic page exceeds the image byte limit.");
            }
            ...
            if (bitmap is null) {
                throw new InvalidDataException("A comic page could not be decoded.");
            }
        }
    }
