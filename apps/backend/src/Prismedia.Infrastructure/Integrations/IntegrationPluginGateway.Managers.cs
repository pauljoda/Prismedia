...
public sealed partial class IntegrationPluginGateway : IIntegrationManagerGateway, IIntegrationLibraryGateway {
    ...
    /// <inheritdoc />
    public async Task<ProviderLibraryCatalog> ListLibrariesAsync(string pluginId, IntegrationConnectionContext connection, CancellationToken cancellationToken) =>
        await InvokeAsync<ConnectionProbeInput, ProviderLibraryCatalog>(await RequireManagerAsync(pluginId, cancellationToken), IntegrationOperation.ListLibraries, connection, new(), cancellationToken);
