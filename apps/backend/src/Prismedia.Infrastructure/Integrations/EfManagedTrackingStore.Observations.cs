...
/// <summary>Maps manager file snapshots onto local library sources and verifies their bytes stay unchanged.</summary>
public sealed partial class EfManagedTrackingStore {
    ...
    public async Task<ManagedTrackingObservation> ObserveAsync(Guid connectionId, ManagedItemSnapshot snapshot, CancellationToken token) {
        ...
        return new(roots.Length == 1 ? roots[0] : null, observed,
            await SourcesAsync(observed.Select(file => file.LocalPath).ToArray(), token));
    }
    ...
    private async Task<IReadOnlyList<ManagedLocalSource>> SourcesAsync(IReadOnlyList<string> paths, CancellationToken token) {
        var owners = await (from file in db.EntityFiles.AsNoTracking()
                            ...
                            where paths.Contains(file.Path)
        ...
        var ids = owners.Select(owner => owner.EntityId)
            .Concat(owners.Where(owner => owner.ParentEntityId != null).Select(owner => owner.ParentEntityId!.Value)).Distinct().ToArray();
        ...
        return owners.Select(owner => {
            int? Own(string code) =>
                positions.FirstOrDefault(position => position.EntityId == owner.EntityId && position.Code == code)?.Value;
            var season = Own(EntityPositionCodes.Season)
                ?? positions.FirstOrDefault(position => position.EntityId == owner.ParentEntityId
                    && position.Code == EntityPositionCodes.Season)?.Value;
            var issueLabel = positions.FirstOrDefault(position => position.EntityId == owner.EntityId
                && position.Code == EntityPositionCodes.Chapter)?.Label;
