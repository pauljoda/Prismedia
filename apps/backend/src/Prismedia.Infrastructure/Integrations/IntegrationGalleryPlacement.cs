...
public sealed class IntegrationGalleryPlacement(ILibraryFileMutationGuard mutations) : IIntegrationGalleryPlacement {
    ...
    public async Task<PlacedIntegrationGallery?> ReadPlacedAsync(Guid operationId, IntegrationTransferPlan plan, LibraryRootData root,
        ...
        if (File.Exists(target)) {
            throw new InvalidDataException("A gallery placement directory conflicts with an existing file.");
        }
    ...
    /// <inheritdoc />
    public async Task<PlacedIntegrationGallery> PlaceAsync(IntegrationGalleryPlacementRequest request,
        ...
        RejectLink(target);
        RejectLink(pending);
        if (File.Exists(target) || File.Exists(pending)) {
            throw new InvalidDataException("A gallery placement directory conflicts with an existing file.");
        }
    ...
    private static PlacedIntegrationGallery Result(string folder, IntegrationGalleryOutputSet outputs) =>
        new(folder, outputs.Artifacts.ToDictionary(artifact => artifact.Id,
            artifact => Path.Combine(folder, IntegrationGalleryNames.FileName(artifact))));
    ...
    private static async Task CopyAsync(VerifiedIntegrationArtifact artifact, string target, CancellationToken token) {
        ...
        if (File.Exists(target)) {
            await VerifyFileAsync(target, artifact, token);
            return;
        }
        ...
        await using (var output = new FileStream(pending, FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.None, 128 * 1024, true)) {
            ...
            if (input.Length != artifact.SizeBytes) {
                throw new InvalidDataException("Staged gallery bytes changed before placement.");
            }
            ...
            while (true) {
                ...
                if (output.Position + count > artifact.SizeBytes) {
                    throw new InvalidDataException("Staged gallery bytes exceed their accepted length.");
                }
            ...
            if (output.Length != artifact.SizeBytes
    ...
    private static async Task VerifyFileAsync(string path, VerifiedIntegrationArtifact artifact, CancellationToken token) {
        RejectLink(path);
        if (!File.Exists(path)) {
            throw new InvalidDataException("An accepted gallery member is missing.");
        }
        ...
        if (stream.Length != artifact.SizeBytes
            || !Convert.ToHexStringLower(await SHA256.HashDataAsync(stream, token))
    ...
    private static void ValidateOutputs(IntegrationGalleryOutputSet outputs) {
        if (outputs.Artifacts.Count == 0
            || outputs.Artifacts.Select(item => item.Id).Distinct(StringComparer.Ordinal).Count() != outputs.Artifacts.Count) {
