...
public sealed class TransferPlanProtector {
    ...
    public string Unprotect(Guid connectionId, Guid operationId, string value) {
        try { return provider.CreateProtector(Purpose, connectionId.ToString("N"), operationId.ToString("N")).Unprotect(value); }
        catch (CryptographicException) { throw new IntegrationTransferPlanUnavailableException(); }
    }
}
