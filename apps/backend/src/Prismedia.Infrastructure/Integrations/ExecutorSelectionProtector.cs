...
public sealed class ExecutorSelectionProtector(string dataDirectory) : IExecutorSelectionProtector {
    ...
    public string Protect(Guid connectionId, AcceptedExecutorSelection selection) {
        ...
        if (token.Length > 262144) {
            throw new IntegrationInvocationException("The inspected publication choices exceed the host selection limit.");
        }
    ...
    public AcceptedExecutorSelection Read(Guid connectionId, string token) {
        if (string.IsNullOrWhiteSpace(token) || token.Length > 262144) {
            throw InvalidSelection();
        }

        try {
            return JsonSerializer.Deserialize<AcceptedExecutorSelection>(Protector(connectionId).Unprotect(token),
                PluginProcessTransport.JsonOptions) ?? throw InvalidSelection();
        } catch (Exception error) when (error is CryptographicException or JsonException) {
            throw InvalidSelection();
        }
    }

    private ITimeLimitedDataProtector Protector(Guid id) =>
        provider.CreateProtector(Purpose, id.ToString("N")).ToTimeLimitedDataProtector();
