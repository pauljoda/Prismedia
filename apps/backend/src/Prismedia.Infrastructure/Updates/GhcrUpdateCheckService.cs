...
namespace Prismedia.Infrastructure.Updates;

/// <summary>Host paths the update check needs, provided by the API host at startup.</summary>
/// <param name="ContentRootPath">The host content root, used to locate the local package.json version.</param>
public sealed record UpdateCheckOptions(string ContentRootPath);
...
/// </remarks>
public sealed class GhcrUpdateCheckService : IUpdateCheckService {
    /// <summary>Named HTTP client the API host registers for registry calls.</summary>
    public const string HttpClientName = "PrismediaUpdateCheck";
    ...
    private async Task<UpdateCheckResponse> FetchChannelStatusAsync(
        ...
        var localVersion = ResolveCurrentVersion(_configuration, _options.ContentRootPath);
    ...
    public static int? CompareVersions(string latestVersion, string localVersion) {
