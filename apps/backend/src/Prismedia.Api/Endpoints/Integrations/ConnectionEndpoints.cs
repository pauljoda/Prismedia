...
public static class ConnectionEndpoints {
    ...
    public static RouteGroupBuilder MapConnectionEndpoints(this IEndpointRouteBuilder routes) {
        ...
        group.MapPut("/{id:guid}", async (Guid id, UpdateConnectionRequest request, ConnectionService service,
            CancellationToken cancellationToken) =>
        ...
        group.MapDelete("/{id:guid}", async (Guid id, long expectedRevision, ConnectionService service,
            CancellationToken cancellationToken) => {
        ...
        group.MapPost("/{id:guid}/acquire-executor", async (Guid id, AcquireExecutorItemRequest request,
            ...
            .AddEndpointFilter<IntegrationTransferProblemFilter>();
        group.MapPost("/{id:guid}/library/search", async (Guid id, ManagedLibraryQuery request, ManagedLibraryService service,
            CancellationToken cancellationToken) =>
        ...
        group.MapPost("/{id:guid}/library/item", async (Guid id, ManagedItemInput request, ManagedLibraryService service,
            CancellationToken cancellationToken) =>
        ...
        group.MapPost("/{id:guid}/manager/options", async (Guid id, ManagerOptionsInput request, ManagedLibraryService service,
            CancellationToken cancellationToken) =>
        ...
        group.MapPost("/{id:guid}/library/mounts", async (Guid id, CreateExternalLibraryMountRequest request,
            ExternalLibraryService service, CancellationToken cancellationToken) =>
            Results.Ok(await service.CreateAsync(id, request, cancellationToken)))
            .WithName("CreateExternalLibraryMount").Produces<ExternalLibraryMount>().Produces<ApiProblem>(400)
            .Produces<ApiProblem>(409);
        group.MapPost("/{id:guid}/library/mounts/attach", async (Guid id, AttachExistingExternalLibraryMountRequest request,
            ExternalLibraryService service, CancellationToken cancellationToken) =>
            Results.Ok(await service.AttachAsync(id, request, cancellationToken)))
            .WithName("AttachExistingExternalLibraryMount").Produces<ExternalLibraryMount>().Produces<ApiProblem>(400)
            .Produces<ApiProblem>(409);
        group.MapPost("/{id:guid}/library/local-access", async (Guid id, ManagedItemInput request, ExternalLibraryService service,
            CancellationToken cancellationToken) =>
            Results.Ok(await service.InspectAsync(id, request, cancellationToken)))
            .WithName("InspectExternalLibraryAccess").Produces<MappedLibrarySnapshot>().Produces<ApiProblem>(400);
        group.MapPost("/{id:guid}/library/tracking-preview", async (Guid id, ManagedItemInput request, ManagedTrackingService service,
        ...
        group.MapPost("/{id:guid}/library/tracking", async (Guid id, TrackManagedHoldingRequest request, ManagedTrackingService service,
            CancellationToken token) =>
        ...
        group.MapPost("/{id:guid}/library/tracking/{holdingId:guid}/refresh", async (Guid id, Guid holdingId,
            ManagedTrackingService service, CancellationToken token) => {
    ...
    private sealed class ConnectionProblemFilter : IEndpointFilter {
        ...
        public async ValueTask<object?> InvokeAsync(EndpointFilterInvocationContext context, EndpointFilterDelegate next) {
            try {
                ...
                return Results.NotFound(new ApiProblem(ApiProblemCodes.ConnectionNotFound, error.Message));
            } catch (ConnectionInUseException error) {
                return Results.Conflict(new ApiProblem(ApiProblemCodes.ConnectionInUse, error.Message));
            } catch (ConnectionConflictException error) {
                return Results.Conflict(new ApiProblem(ApiProblemCodes.ConnectionConflict, error.Message));
            } catch (ManagedControlConflictException error) {
                return Results.Conflict(new ApiProblem(ApiProblemCodes.ManagedControlConflict, error.Message));
            } catch (ManagedRequestConflictException error) {
                return Results.Conflict(new ApiProblem(ApiProblemCodes.ManagedRequestConflict, error.Message));
                ...
                return Results.BadRequest(new ApiProblem(ApiProblemCodes.ConnectionInvalid, error.Message));
            } catch (ConnectionSecretUnavailableException error) {
                return Results.BadRequest(new ApiProblem(ApiProblemCodes.ConnectionUnavailable, error.Message));
