...
public static class ManagedDiscoveryEndpoints {
    ...
    public static void MapManagedDiscoveryEndpoints(this RouteGroupBuilder connections) {
        ...
        group.MapPost("/review", async (Guid id, ManagedDiscoveryReviewRequest request, ManagedDiscoveryService service,
        ...
        group.MapPost("/prepare", async (Guid id, PrepareManagedDiscoveryRequest request, ManagedDiscoveryService service,
            CancellationToken token) => {
            try {
                return Results.Ok(await service.PrepareAsync(id, request, token));
