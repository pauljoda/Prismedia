...
public sealed class IntegrationConnection(IntegrationConnectionState state) {
    ...
    public void Configure(string name, string baseUrl, bool enabled, IReadOnlyList<PluginCapability> capabilities,
        ...
        if (string.IsNullOrWhiteSpace(name) || name.Trim().Length > 200) {
            throw new ArgumentException("A connection name of at most 200 characters is required.");
        }
        ...
        if (capabilities is not { Count: > 0 and <= 8 } || capabilities.Distinct().Count() != capabilities.Count
            || capabilities.Any(capability => !Enum.IsDefined(capability)
                || !PluginCapabilityDefinition.For(capability).EnabledPerConnection)) {
        ...
        if (settings is null || settings.Count > 32 || settings.Any(pair => string.IsNullOrWhiteSpace(pair.Key)
            || pair.Key.Length > 100 || pair.Value is null || pair.Value.Length > 4096)) {
            throw new ArgumentException("Connection settings exceed their limits.");
        }
        ...
        if (State.RemoteInstanceId is not null
            && !string.Equals(new Uri(State.BaseUrl).AbsoluteUri, endpoint.AbsoluteUri, StringComparison.Ordinal)) {
    ...
    #region Actions - Health

    /// <summary>Records a probe while retaining established identity across outages and remote replacement.</summary>
    public void RecordProbe(string? remoteInstanceId, IReadOnlyList<IntegrationSupport> effective, string? error, DateTimeOffset now,
        bool hasPersistentRemoteIdentity = true) {
        var changedIdentity = State.RemoteInstanceId is not null
            && !string.Equals(State.RemoteInstanceId, remoteInstanceId, StringComparison.Ordinal);
        ...
        State = State with {
            ...
            HasPersistentRemoteIdentity = State.RemoteInstanceId is not null
                ? State.HasPersistentRemoteIdentity
                : error is null && hasPersistentRemoteIdentity,
            EffectiveCapabilities = status == ConnectionStatus.Ready ? effective.ToArray() : [],
            LastError = error ?? (changedIdentity
                ? "The remote installation identity changed. Create a new connection before using this application."
