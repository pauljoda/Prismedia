...
public static class ManagedSourceAdoption {
    ...
    public static ManagedSourceAdoptionPlan Plan(IReadOnlyList<ManagedObservedFile> observed,
        ...
        if (selected.Count != targets.Length
            || selected.Select(item => item.RemoteTargetId).Distinct(StringComparer.Ordinal).Count() != targets.Length
        ...
        foreach (var file in observed) {
            ...
            if (owners.Length != file.Targets.Count) {
                return Review("The local source has different ownership coverage from the connected application.");
            }
            ...
            foreach (var target in file.Targets) {
                ...
                var matches = owners.Where(owner => selection is not null && owner.EntityId == selection.EntityId
                    && owner.SourceFileId == selection.SourceFileId).ToArray();
                if (matches.Length != 1) {
                    return Review("A selected local source changed. Refresh the file matches before linking.");
                }
