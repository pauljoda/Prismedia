...
public sealed class IntegrationArtifactManifest {
    ...
    /// <summary>
    /// Accepts only a complete sealed revision; missing pages, duplicate identities, unsafe paths, and invalid hashes are rejected.
    /// </summary>
    public IntegrationArtifactManifest(string jobId, string revision, bool isSealed, int artifactCount,
        IReadOnlyList<IntegrationArtifact> artifacts) {
        ...
        foreach (var artifact in artifacts) {
            if (artifact is null || string.IsNullOrWhiteSpace(artifact.Id) || artifact.Id.Length > 512 || !ids.Add(artifact.Id)
                ...
                || string.IsNullOrWhiteSpace(artifact.MediaType) || artifact.MediaType.Length > 256
                ...
                throw new ArgumentException("The artifact manifest contains invalid or duplicate byte evidence.");
            }
            ...
            if ((artifact.GroupId is null) != (artifact.Ordinal is null) || artifact.GroupId is { } group
                && (string.IsNullOrWhiteSpace(group) || group.Length > 512 || artifact.Ordinal is < 1
                    || !positions.Add((group, artifact.Ordinal!.Value)))) {
    ...
    private static bool IsPortablePath(string? path) => !string.IsNullOrWhiteSpace(path) && path.Length <= 2048
        ...
        && !path.Any(character => char.IsControl(character) || character is ':' or '*' or '?' or '"' or '<' or '>' or '|')
        && path.Split('/').All(segment => segment.Length is > 0 and <= 255 && segment is not ("." or "..")
