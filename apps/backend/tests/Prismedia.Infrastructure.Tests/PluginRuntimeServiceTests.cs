...
public sealed class PluginRuntimeServiceTests : IDisposable {
    ...
    [Fact]
    public async Task IdentifyMusicArtistDoesNotCascadeIntoAlbumsOrTracks() {
        // Identifying an artist must not fan out into a provider request per album and per track —
        // that storm is what rate-limited providers reject. Only the artist itself is requested.
        ...
        await File.WriteAllTextAsync(
            ...
            """
            {
              ...
              "auth": [],
              "supports": [
                { "entityKind": "music-artist", "actions": ["lookup-id", "search"] },
                { "entityKind": "audio-library", "actions": ["lookup-id", "search"] },
                { "entityKind": "audio-track", "actions": ["lookup-id", "search"] }
              ]
            }
            """);
        ...
        var artistId = Guid.Parse("11111111-aaaa-4444-8888-111111111111");
        var albumId = Guid.Parse("22222222-aaaa-4444-8888-222222222222");
        var trackId = Guid.Parse("33333333-aaaa-4444-8888-333333333333");
        db.Entities.AddRange(
            ...
            new EntityRow { Id = trackId, KindCode = "audio-track", Title = "Believer", ParentEntityId = albumId, SortOrder = 0, CreatedAt = now, UpdatedAt = now });
        await db.SaveChangesAsync();

        var executor = new MusicArtistShellProcessExecutor();
        var service = CreateIdentifyService(db, executor, pluginDir);
        ...
        // Only the artist was sent to the plugin — no album or track cascade.
        Assert.Equal([artistId], executor.Requests.Select(request => request.Entity.Id).ToArray());
        Assert.Empty(response.Result!.Children);
    }

    private sealed class MusicArtistShellProcessExecutor : ProcessExecutor {
        ...
        public override async Task<ProcessExecutionResult> RunAsync(
            ...
            var proposal = new EntityMetadataProposal(
                "musicbrainz:artist:imagine-dragons",
                "musicbrainz",
                "music-artist",
                0.9m,
                "external-id",
                EmptyPatch() with {
                    ...
                    ExternalIds = new Dictionary<string, string> { ["musicbrainz"] = "012151a8" }
                },
                [],
                [],
                []);
