...
async function main() {
  ...
  const entityKindEntries = (manifest.entityKinds ?? []).map((kind) =>
    ...
    `modalities: ${lit(kind.modalities)}, ` +
  ...
  // Library-root scan requirement per kind. Capability codes match library-root property names.
  ...
  // hand-listing which phases are active, settled, cancellable, or need attention.
  if (!manifest.closedSetFacts) throw new Error("Manifest is missing closed-set facts");
  for (const enumName of Object.keys(manifest.closedSetFacts).sort()) {
    const constName = `${enumName.replace(/([a-z0-9])([A-Z])/g, "$1_$2").toUpperCase()}_FACTS`;
    sections.push(
      `// source: ${enumName}Definition\n` +
      `export const ${constName} = ${JSON.stringify(manifest.closedSetFacts[enumName], null, 2)} as const satisfies Record<${enumName}Code, Record<string, boolean>>;\n`,
    );
  }
