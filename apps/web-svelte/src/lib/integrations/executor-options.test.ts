...
describe("executor destinations", () => {
  ...
  it("uses writable image libraries for images and publication libraries for books", () => {
    ...
    expect(executorRoots(roots, ENTITY_KIND.gallery).map(root => root.id)).toEqual(["images"]);
    expect(executorRoots(roots.map(root => ({ ...root, recursive: false })), ENTITY_KIND.gallery)).toEqual([]);
  });
