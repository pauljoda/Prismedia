<script lang="ts">
  ...
  import { Button, Checkbox, ChoiceGroup, SearchableSelect, Select, TextInput } from "@prismedia/ui-svelte";
...
<div class="flex flex-col gap-3">
  <ChoiceGroup type="single" ariaLabel="Rule editor mode" size="sm"
    options={[{ value: String(false), label: "Basic" }, { value: String(true), label: "Advanced" }]}
    value={String(advanced)} onValueChange={(value) => (advanced = value === String(true))} />
  {#if !advanced}
    <div class="flex flex-col gap-2">
      <div class="flex flex-col gap-1">
        <span class="text-label text-text-secondary">Start with a common preference</span>
        <SearchableSelect label="Common preference" searchLabel="Find a language or format" value={presetIndex}
          options={presets.map((preset, index) => ({ value: String(index), label: preset.name }))}
          onchange={(value) => (presetIndex = value)} />
      </div>
    ...
    <div class="flex flex-col gap-2 border-t border-border-subtle pt-3">
      <label class="block flex flex-col gap-1"><span class="text-label text-text-secondary">Or match text in the release title</span>
  ...
  {:else}
    ...
    {#each form.conditions as condition, index (index)}
      <div class="flex flex-col gap-2 rounded-sm border border-border-subtle p-3">
        ...
        <div class="flex flex-wrap items-center gap-4">
          <label class="flex items-center gap-2 text-xs text-text-secondary"><Checkbox checked={condition.required} onchange={(checked) => (form.conditions[index].required = checked)} />Required</label>
          <label class="flex items-center gap-2 text-xs text-text-secondary"><Checkbox checked={condition.negate} onchange={(checked) => (form.conditions[index].negate = checked)} />Exclude</label>
